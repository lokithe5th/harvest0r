...
contract Seeds is ERC721A {
  ...
  function generateSVGofTokenById(uint256 tokenId) public view returns (string memory svg) {
    svg = string(abi.encodePacked(svgParts[0], viewCharge(tokenId), svgParts[1], svgParts[2]));
  }
  ...
  function viewCharge(uint256 tokenId) public view returns (string memory) {
    return (uint256(charges[tokenId])).toString();
  }
}
